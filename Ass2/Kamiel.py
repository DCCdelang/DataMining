...
def grid_search():
    '''
    best = xgb.XGBRegressor(objective ='reg:linear', colsample_bytree = 0.3, learning_rate = 0.1, max_depth = 4, alpha = 10, n_estimators = 150)
    '''
    train = pd.read_csv('Data/fifty_fifty_small.csv')
    train = train.fillna(-1)
...
def test_reg_model(reg):
    ...
    test = pd.read_csv('Data/test_validation.csv')
...
def make_submission_file():
    ...
    df_sub.to_csv('Data/submission_GXBOOST_new_features.csv', index=False)
